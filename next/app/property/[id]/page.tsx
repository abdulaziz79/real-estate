    ...
    <div className="pt-20 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <div className="flex gap-2 mb-4">
              ...
              <span className={`px-4 py-1 rounded-full text-xs font-bold uppercase ${property.type === "Under Construction" ? "bg-orange-500 text-white" : "bg-white text-slate-700 border border-slate-200"}`}>{property.type}</span>
              {property.isBoosted && (
            ...
            <p className="text-slate-500 text-lg flex items-center gap-2">
              <svg className="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /></svg>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-16">
          ...
          <div className="lg:col-span-3 flex lg:flex-col gap-4 overflow-x-auto lg:overflow-y-auto max-h-[550px] pb-4 lg:pb-0">
          ...
          <div className="lg:col-span-2 space-y-16">
            ...
            <section>
              ...
              <div className="bg-white rounded-[3.5rem] border-8 border-white shadow-2xl overflow-hidden h-[500px] group">
                <iframe title="Google Maps" width="100%" height="100%" className="grayscale-[0.2] contrast-[1.1] group-hover:grayscale-0 transition-all duration-700" src={fallbackMapUrl} />
              </div>
              <div className="mt-6 flex items-center justify-between bg-white px-8 py-4 rounded-2xl border border-slate-200">
                ...
                <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${property.city}, ${property.country}`)}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 font-bold hover:underline flex items-center gap-1">
                  Open in Google Maps
            ...
            {property.type === "Under Construction" && property.monthlyInstallment ? (
              <section className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-[3.5rem] p-12 text-white shadow-2xl relative overflow-hidden">
                <h2 className="text-4xl font-bold mb-10 flex items-center gap-4 relative z-10">Payment Plan</h2>
              ...
              <section className="bg-slate-900 rounded-[3.5rem] p-12 text-white shadow-2xl relative overflow-hidden">
                <h2 className="text-4xl font-bold mb-10 flex items-center gap-4">Financial Planner</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-16 relative z-10">
                  ...
                  <div className="flex flex-col justify-center items-center p-12 bg-gradient-to-br from-blue-600 to-blue-700 rounded-[3rem] shadow-2xl scale-105 border-4 border-white/10">
                    ...
                    <p className="mt-6 text-white/60 text-xs text-center leading-relaxed">Estimations based on typical global lending rates.</p>
                  </div>
                </div>
              </section>
            )}
          ...
          <div className="space-y-8">
            {agent && (
              <div className="bg-white rounded-[3rem] p-8 border border-slate-200 shadow-xl sticky top-24">
                ...
                <Link href={`/agents/${agent.id}`} className="flex items-center gap-6 mb-8 hover:bg-slate-50 p-4 -mx-4 rounded-[2rem] transition-all group">
                ...
                <form className="space-y-4 mb-8">
                  ...
                  <textarea placeholder="Tell us what you're looking for..." rows={4} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 outline-none resize-none focus:ring-2 focus:ring-blue-500 transition-all" />
                  <button type="button" className="w-full bg-blue-600 text-white font-bold py-5 rounded-2xl hover:bg-blue-700 transition-all shadow-xl hover:-translate-y-1 active:translate-y-0">Request Portfolio</button>
                </form>
                <div className="flex flex-col gap-3">
                  <button type="button" className="w-full bg-slate-900 text-white font-bold py-5 rounded-2xl hover:bg-slate-800 transition-all flex items-center justify-center gap-3">
