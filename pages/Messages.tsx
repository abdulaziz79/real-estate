...
export const Messages: React.FC = () => {
  ...
  useEffect(() => {
    if (location.state?.targetId) {
      setActiveChat({ id: location.state.targetId, name: location.state.targetName });
    }
  }, [location.state]);
        ...
        <div className="divide-y divide-slate-50">
          {activeChat ? (
            <div className="p-6 bg-blue-50 border-l-4 border-blue-600 flex gap-4 cursor-pointer">
              <div className="w-12 h-12 bg-blue-200 rounded-full flex items-center justify-center font-bold text-blue-600">
        ...
        {activeChat ? (
          <>
            <div className="bg-white p-6 border-b border-slate-200 flex items-center gap-4">
               <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center font-bold text-slate-600 md:hidden">
                {activeChat.name.charAt(0)}
              </div>
            ...
            <form onSubmit={sendMessage} className="p-6 bg-white border-t border-slate-200">
               <div className="max-w-4xl mx-auto flex gap-4">
                  <input 
                    ...
                    className="flex-1 bg-slate-100 border-none rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-blue-500"
                  />
                  <button className="bg-blue-600 text-white p-4 rounded-2xl shadow-lg hover:bg-blue-700 transition-all">
