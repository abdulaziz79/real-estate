...
export const AgentDetail: React.FC = () => {
  ...
  const [agent, setAgent] = useState(MOCK_AGENTS.find(a => a.id === id));
  ...
  const handleMessage = () => {
    ...
    navigate('/messages', { state: { targetId: agent.userId, targetName: agent.name } });
  };
              ...
              {user && user.role === 'user' && (
                <form onSubmit={submitReview} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm mb-8">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 outline-none focus:ring-2 focus:ring-blue-500 mb-4"
                    placeholder="Share your experience..."
                    rows={3}
                    required
                  />
                  <button className="bg-slate-900 text-white font-bold px-8 py-3 rounded-xl hover:bg-slate-800 transition-all">Submit Feedback</button>
                </form>
              )}
