            ...
            <div key={agent.id} className="bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
              ...
              <div className="relative">
                ...
                <div className="space-y-4 mb-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {agent.specialties.map((spec) => (
                      <span key={spec} className="bg-slate-100 text-slate-600 px-3 py-1 rounded-lg text-xs font-medium">{spec}</span>
                    ))}
                  </div>
                </div>
                <div className="flex items-center justify-between border-t border-slate-100 pt-6">
                  ...
                  <Link href={`/agents/${agent.id}`} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-600 transition-all shadow-md">View Profile</Link>
                </div>
              </div>
            </div>
          ))}
