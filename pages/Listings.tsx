...
import { MOCK_PROPERTIES, GEOGRAPHY, COUNTRIES } from '../constants';
...
export const Listings: React.FC = () => {
  ...
  const [filters, setFilters] = useState({
    type: (searchParams.get('type') as PropertyType) || 'All' as PropertyType | 'All',
  ...
  const filteredProperties = useMemo(() => {
    return allProperties
      ...
      .filter(p => {
        ...
        const matchBeds = filters.beds === 'All' || p.bedrooms >= parseInt(filters.beds);
  ...
  const updateFilters = (newFilters: Partial<typeof filters>) => {
    ...
    if (updated.country) params.country = updated.country;
    if (updated.city) params.city = updated.city;
    if (updated.type !== 'All') params.type = updated.type;
    if (updated.beds !== 'All') params.beds = updated.beds;
    if (updated.minPrice > 0) params.minPrice = updated.minPrice.toString();
    if (updated.maxPrice < 10000000) params.maxPrice = updated.maxPrice.toString();
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6">
            <div>
              ...
              <select 
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 focus:ring-2 focus:ring-blue-500 outline-none appearance-none font-bold"
                value={filters.country}
                onChange={(e) => updateFilters({ country: e.target.value, city: '' })}
              >
            ...
            <div className="lg:col-span-2">
              ...
              <input 
                type="text" 
                placeholder="View, Pool, Beachfront..."
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 focus:ring-2 focus:ring-blue-500 outline-none font-bold"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex justify-between items-center mb-10">
          <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">
            Found <span className="text-slate-900">{filteredProperties.length}</span> Verified Assets
          </p>
        ...
        {filteredProperties.length > 0 ? (
          ...
          <div className="text-center py-32 bg-white rounded-[4rem] border-4 border-dashed border-slate-100">
            ...
            <button 
              onClick={() => updateFilters({ type: 'All', country: '', city: '', minPrice: 0, maxPrice: 10000000, beds: 'All', search: '' })}
              className="bg-slate-900 text-white px-10 py-4 rounded-[2rem] font-black uppercase tracking-widest text-xs shadow-2xl hover:bg-slate-800 transition-all"
            >
              Reset Filters
            </button>
          </div>
